...
public enum DMS {
    ...
    public static func parseDMS(degMinSec: String) -> Double {
        ...
        var results = zip(matches, matches.dropFirst().map { Optional.some($0) } + [nil]).map{ current, next -> String in
            ...
            let start = String.Index(utf16Offset: newRange.location, in: dms)
            let end = next.map {$0.range(at: 0)}.map { String.Index(utf16Offset: $0.location, in: dms)} ?? String.Index(utf16Offset: dms.utf16.count, in: dms)
