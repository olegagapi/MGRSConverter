...
//            }
        ...
        var results = zip(matches, matches.dropFirst().map { Optional.some($0) } + [nil]).map{ current, next -> String in
            ...
            let start = String.Index(utf16Offset: newRange.location, in: mgrsString)
            let end = next.map {$0.range(at: 0)}.map { String.Index(utf16Offset: $0.location, in: mgrsString)} ?? String.Index(utf16Offset: mgrsString.utf16.count, in: mgrsString)
